...
| `vitro/engine/vm` | L7 | **执行器状态机 + 快照体系 + executor 段一**（批一号 = 状态定形 + 批二段一 = step/run + 单层穷尽 match 135 臂 + 八族 + trap 渲染单点 + base_kind 上提 ast，2026-09-25；F/D/Q 三族与控制流族随批二段二）：`VitroVM`（值栈 u64 位模式 / 调用栈〔宿主回调哨兵 = `return_ip : Int?`〕/ 教学观测 / 宿主域三态 / 装载原语——`register_function`〔MAX_FUNCTIONS 上限〕/`set_symbols`〔派生索引单点〕/`setup_argv`〔R1 ③〕/`write_cstring`〔cstring 单源〕）+ `VMSnapshot`/`MemoryImage` 定形（两端单点 + 拷贝即快照）+ **C# 异常三执行状态**（handler 栈 / 异常寄存器 / UNWINDING——CS 硬前置 v1 入形，一等进快照）+ ARC 帧退出清理占位（`arc_cleanup`，C 恒空）。**新设计裁剪**：JIT 全族（F-3）/ `global_count` / C++ 守卫 / `trace` 死路径 / sym_map 常驻索引（→ 装载期派生 `locals_by_func`/`global_syms`，消坑 12 热点）。24 测试（快照 8 + executor 段一 14 + 黑盒 2） | 🔨 S6 vm 批一号 + 批二段一已落（2026-09-25；批二段二 F/D/Q 三族 + 控制流族 / 批三号控制流回调族 9 件 / 批四号 libc 自举 + CheckpointManager / 批五号 D 级三联 diff + 门 3 + 条件 A → 发布 + 0.6.0 窗口） |
