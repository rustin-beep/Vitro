...
> 流水线图：![影子验证门禁流水线](shadow-verification-flow.svg)——图内快照数字经 `data-fact` 锚定
> `reports/facts.json`，由 `go run ./scripts/gen_svg` 生成，漂移由 `go run ./scripts/facts check` 机判（勿手改图内锚定数字）。
