...
1. **面闸口径修复（P7 盲区）——`moonbit_surface` 三处口径不一致**（实测：① 消费侧取目录末段 `filepath.Base(filepath.Dir(f))`，而 corpus 是**全递归** walk——含 `lexer/internal/host` 等子包文件；② 提供侧 `filepath.Glob("*/pkg.generated.mbti")` **只扫一层**——全仓 17 个包接口面有 **4 个子包**（`lexer/internal/host`、`lexer/internal/pp`、`lexer/internal/scanner`、`lexer/token`）pub 面零扫描；③ 边表两侧末段形式，同名条目只能靠注释区分）。当前危害 =「**少收**」（安全侧：`lexer/internal/host` 的 `@host.` 引用会救活 L7 `host` 同名符号的消费判定，该收的 pub 收不掉）；**触发线 = L7 host 扩充期**——余量批 ~106 handler 与 VFS/存根名域邻近 L4 的 `stub_lookup`/`stubs_provider`/`vfs_provider`。⇒ **批一段（口径修复）排在 S6 余量批（host ~106 handler + VFS）开工之前**：consumer/provider 改包全名（全仓 `moon.pkg` 零显式别名，按「别名 = import 路径末段」实现 + fail loud 未知形态）+ 边表/白名单全名化重刷；验收 = J9 同名符号对证红（修复前误判、修复后分离）+ 重刷后 `-check` 全绿。**批二段（Glob 改全递归 = 4 子包入闸）是行为扩张**——子包 pub 面首次进「无主须收面或入白名单」判定、会暴露一批待收符号 ⇒ 排在 **0.6.0 发布前**（与既有收面义务合批，出口 = 可收清单清零或入白名单）。文件：`scripts/moonbit/moonbit_surface/moonbit_surface.go` / `surface_edges.txt` / `surface_allowlist.txt`。
