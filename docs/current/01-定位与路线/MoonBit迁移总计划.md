...
```
L0 零依赖   vitro/engine/source(SourceLoc+坐标契约)   vitro/engine/opcode(135+Instruction+operand 校验；44–46 = C# 异常三件 TryBegin/TryEnd/Throw 进历史空号，2026-09-25 vm 批一号)
...
**验收口径**：Rust 侧每条防线，要么在 MoonBit 侧找到对等物，要么显式裁定不需要——补语义覆盖面，不补行数。实测基数：native/tests 52 文件 23,684 行 vs MoonBit 测试 11,767 行 / 434 个；其中 ~16k Rust 单元/管线测试的对等物 = S2–S6 白盒锚 + 四级对拍（已付讫）；语料 708 个 .c 双方共用不消失。真净缺口与承接（五层，按性价比排序）：
...
| 2 | **Clang 直拍**（终态常驻防线）：shadow_verify 的 Clang 侧设施（.clang_cache 缓存/并发/重试）全复用，被测物从 DLL 换 cmd/run exe，输出比对复用 vm_diff 提取器——新写仅绑定层；known_issue 台账新建（DIFF-LIB-PUTCHAR-01 配方：gap + @category + clang golden + 双向监控）。真值强度高于对拍 Rust | 新 Go 驱动（复用 internal/canonicalize） | S7 稳定片期 |
...
| G-5 | **成员偏移单点化**（kimicc 调查输入，2026-09-26） | `codegen/index_member.mbt:11-45`（`get_member_offset`）与 `codegen/addr.mbt:78-84` 两份 inline 累加 → 抽 `field_offset(struct,name)` 单点（外部先例：kimicc `FieldLayout/offset_of_path` 单点，[调查报告](../07-质量与裁定/kimicc外部参考调查报告20260926.md) §3.1）；A 级对拍 598/598 全量兜底，低风险 | 随下个 codegen 触点批（0.6.0 发版后） |
| G-6 | **printf/libc 事实重组**（kimicc 调查输入，2026-09-26） | typeck `check_printf_format` / codegen `bytecode/route.mbt` / vm `host_dispatch.mbt` 三处分居 → 参照 kimicc ctype `_info/_arg_type` 成对纯函数模式（`cfront/ctype/builtin.mbt:796-1576`）重组为单源词汇点（[调查报告](../07-质量与裁定/kimicc外部参考调查报告20260926.md) §3.2）；行为等价重构，三包锚全覆盖后才动 | 0.6.0 后独立批（先 G-5 后本项——先消偏移双写小债，再动三包重组） |
