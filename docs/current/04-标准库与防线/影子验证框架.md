...
> 最后核对：2026-09-20（规模口径对齐 2026-09-20 shadow 产物；门禁流程与 §2.3 判定表不变）
>
...
> 当前规模：C 侧 **680 个用例**（完全匹配 676 + known_issue 3 + gap_extension 1，0 非预期差异；vitro_better 已清零），
...
Go 驱动 680 用例实测：冷启动全量重算 ~26s（16 路并发）、缓存热跑 ~5s。
