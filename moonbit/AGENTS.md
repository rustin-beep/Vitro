...
| `vitro/engine/vm` | L7 | **执行器状态机 + 快照体系 + executor 全量**（批一号状态定形 + 批二段一骨架〔八族 + trap 渲染单点 + base_kind 上提〕+ 批二段二〔F/D/Q 三族 + 控制流族：do_call 四重防护/宿主回调哨兵 Ret；rebuild_local_sym_map 热点零调用〕，2026-09-25/26；fail loud 剩 CallHost 与 C# 三件）：`VitroVM`（值栈 u64 位模式 / 调用栈〔宿主回调哨兵 = `return_ip : Int?`〕/ 教学观测 / 宿主域三态 / 装载原语——`register_function`〔MAX_FUNCTIONS 上限〕/`set_symbols`〔派生索引单点〕/`setup_argv`〔R1 ③〕/`write_cstring`〔cstring 单源〕）+ `VMSnapshot`/`MemoryImage` 定形（两端单点 + 拷贝即快照）+ **C# 异常三执行状态**（handler 栈 / 异常寄存器 / UNWINDING——CS 硬前置 v1 入形，一等进快照）+ ARC 帧退出清理占位（`arc_cleanup`，C 恒空）。**新设计裁剪**：JIT 全族（F-3）/ `global_count` / C++ 守卫 / `trace` 死路径 / sym_map 常驻索引（→ 装载期派生 `locals_by_func`/`global_syms`，消坑 12 热点）。80 测试（快照 14〔+门 3 五锚 + 集成版锚 gate3_integrated_roundtrip_with_libc_setup——libc 真装载 + vfs 预设 + 非平凡堆下 1MB 逐字节回滚，2026-09-26 门 3 集成版销账〕+ executor 63〔… + 三段 11：printf/scanf/getchar/puts/call_user_function/qsort×2/bsearch/形状诊断/STEP-guards〕+ 黑盒 2） | 🔨 S6 vm 批一号 + 批二 + 批三号一/二段已落（2026-09-25/26——executor 131/135 臂 + CallHost **110 臂全接线**〔义务①②③全落位〕；批四号 libc 自举 + CheckpointManager / 批五号 D 级三联 diff + 门 3 + 条件 A → 发布 + 0.6.0 窗口） |
...
go run ./scripts/clang_direct [--sample N | --corpus dir | --cases f1.c,...] # §7.1 层 2 Clang 直拍（2026-09-26 提前落地）：真值源=Clang 本尊，cmd/run 的 stdout+返回码 vs Clang golden（**601 例 SAME 596/KNOWN 5/DIFF 0**；known_direct.json case+digest 锁定 + 双向监控；重试判据=shadow 同款〔编译失败也重试——Windows 并发 Defender 锁目标文件首跑 127 例瞬态误判实锤〕；MoonBit 侧 Latin-1 折回归一 + clang 侧文本模式 CRLF 归一；缓存 .clang_cache_cd/；J9 digest 篡改证红留痕）
```
