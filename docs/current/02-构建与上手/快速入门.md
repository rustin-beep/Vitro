...
> 最后核对：2026-09-23（MoonBit 迁移现状对齐——头注补迁移指引；C++ 子集随砍 C++ 裁定归档；断链修复）
>
> **MoonBit 迁移进行中（2026-09-18 起，同仓绞杀者模式）**：本指南覆盖的是**现役 Rust 引擎**（已冻结为差分对照 oracle，在 Rust 区退役前继续可用）；`moonbit/` 活跃区承载全部新开发，其构建与闸门见 [`moonbit/AGENTS.md`](../../../moonbit/AGENTS.md)，迁移进度见 [MoonBit迁移总计划.md](../01-定位与路线/MoonBit迁移总计划.md)。
...
- **MoonBit 迁移（当前工作）**：[MoonBit迁移总计划.md](../01-定位与路线/MoonBit迁移总计划.md)
